"""X/Twitter CTI accounts → The Wire (RSS-shaped Threat ingest)."""
...
from apps.workers.services import is_high_impact_intel, is_vietnam_related

logger = logging.getLogger(__name__)

# Curated dark-web / breach intel voices (extend via X_WIRE_ACCOUNTS).
# Cyber_Citaldel kept as provided; Cyber_Citadel is the common spelling if empty.
...
def _category_for_text(title: str, summary: str) -> str:
...
def _is_important_post(title: str, summary: str) -> bool:
    """Keep CTI signal only — cuts noise and Google Translate spend."""
    text = f"{title} {summary}".casefold()
    if is_high_impact_intel(text):
        return True
    if is_vietnam_related(text):
        return bool(
            re.search(
                r"\b(cyber|security|breach|leak|ransomware|malware|hack|"
                r"attack|threat|extortion|compromise|phishing|exploit|"
                r"defacement|victim|claimed)\b",
                text,
            )
        )
    # Account posts that are clearly claims / dark-web intel without long prose.
    return bool(
        re.search(
            r"\b(claimed|ransom|databreach|data.?breach|dark.?web|stealer|"
            r"infostealer|initial.?access|access.?broker|leak|leaked|"
            r"exposed|dump|victim)\b",
            text,
        )
    )
...
def _hit_to_item(
    ...
    *,
    ...
    return {
        ...
        "category": _category_for_text(title, text),
        "discovery": "x-wire",
        "engine": "x_twitter",
        "x_handle": handle,
        "alleged_claim": True,
...
def discover_x_wire_items(
    *,
    ...
    """Pull important posts from curated X CTI accounts → Wire candidates."""
