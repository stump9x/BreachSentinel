"""Exa → The Wire: turn semantic CTI hits into RSS-shaped Threat ingest rows."""
...
from apps.workers.services import is_high_impact_intel

# Broad CTI queries Exa ranks well on (natural language, not Boolean dorks).
DEFAULT_WIRE_QUERIES = (
    "Latest ransomware attacks and corporate data breaches this week",
    "Latest news on data breaches Vietnam",
    "Latest credential leaks stolen databases and exposed API keys",
)
...
def _category_for_hit(title: str, summary: str) -> str:
...
def _hit_to_item(
    ...
    *,
    ...
    return {
        ...
        "feed": feed,
        "feed_url": feed_url,
        "category": _category_for_hit(title, summary),
        "discovery": discovery,
        "engine": "exa",
    }
...
def discover_exa_wire_news(
    *,
    ...
    """Open-web CTI news via Exa NL queries → Wire candidate items."""
    ...
    max_age = int(getattr(settings, "EXA_WIRE_MAX_AGE_DAYS", 14) or 14)
...
def discover_exa_site_items(
    *,
    ...
    for raw_domain in selected:
        ...
        scanned += 1
        hits = search_exa(
            f"Latest data breaches ransomware leaks on {domain}",
