...
/** Full pin window (hours) — keep in sync with WIRE_VIETNAM_PIN_DAYS (7×24). */
const WIRE_PRIORITY_PIN_HOURS = 7 * 24;
/** Cap for older high-priority rows — keep in sync with WIRE_STALE_PRIORITY_CAP. */
...
export function effectiveWirePriority(row, now = Date.now()) {
  ...
  const when = wireDisplayInstant(row, now);
  if (!when) return Math.min(pri, WIRE_STALE_PRIORITY_CAP);
  const ageHours = (Number(now) - when.getTime()) / 3_600_000;
  if (ageHours <= WIRE_PRIORITY_PIN_HOURS) return pri;
